...
def compute_equity_curve(trades_df: pd.DataFrame, start_equity: float = 100000.0) -> pd.DataFrame:
    if trades_df.empty:
        return pd.DataFrame(columns=["entry_time", "equity", "cummax", "drawdown", "cum_pnl"])
    out = trades_df.sort_values("entry_time").copy()
    out["cum_pnl"] = out["pnl_dollars"].cumsum()
    out["equity"] = start_equity + out["cum_pnl"]
...
def summarize_symbol_result(symbol: str, params: dict, trades_df: pd.DataFrame, daily_df: pd.DataFrame, initial_cash: float) -> dict:
    ...
    equity_df = compute_equity_curve(trades_df, start_equity=initial_cash)
    return {
        "symbol": symbol,
        **params,
...
def run_selector(args: argparse.Namespace) -> None:
    ...
    comparison_path = out_dir / f"comparison_results_{run_ts}.csv"
    ...
    if not grid_df.empty:
        comparison_cols = [c for c in [
            "symbol", "pivot_left", "pivot_right", "min_swing_range_pct", "stop_buffer_pct",
            "max_wait_minutes_after_touch", "max_hold_minutes", "rr_target",
            "avg_daily_return_on_notional", "median_daily_return_on_notional",
            "positive_day_ratio", "total_pnl", "total_trades", "win_rate",
            "equity_final", "max_drawdown"
        ] if c in grid_df.columns]
        grid_df.sort_values(
            by=["avg_daily_return_on_notional", "median_daily_return_on_notional", "positive_day_ratio", "total_pnl"],
            ascending=[False, False, False, False],
        )[comparison_cols].to_csv(comparison_path, index=False)
    else:
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="5.1.1.1 OTE Long Backtest Selector")
