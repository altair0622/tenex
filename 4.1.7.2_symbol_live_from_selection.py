...
def train_model_from_recent_window(
    ...
    model = base.RandomForestClassifier(**base.MODEL_PARAMS)
    model.fit(X_train, y_train.astype(int))
    return model


def latest_signal_probability(
    model,
    df: pd.DataFrame,
...
def main():
    ap = argparse.ArgumentParser(description="4.1.7.2 live runner from coarse-to-fine selection JSON")
    ...
    print(f"symbol={symbol} horizon={horizon} tp={tp} sl={sl} p_enter={p_enter} qty={qty} train_days={train_days} feed={feed}")
    ...
    while True:
        try:
            ...
            if not in_market_window(now):
                ...
                pytime.sleep(max(15, args.poll_seconds))
                continue
            ...
            print(f"[{now.isoformat()}] latest_bar={latest_ts.isoformat()} prob={prob:.4f} price={last_price:.4f} in_position={state['in_position']}")
            ...
            pytime.sleep(max(5, args.poll_seconds))
            ...
            pytime.sleep(max(10, args.poll_seconds))
