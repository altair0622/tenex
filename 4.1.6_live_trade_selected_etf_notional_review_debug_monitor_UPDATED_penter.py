...
# Keep the console readable during live monitoring.
...
def build_thresholds(thresholds_str: str, p_enter: float) -> list[float]:
    vals = []
    for raw in thresholds_str.split(','):
        raw = raw.strip()
        if not raw:
            continue
        try:
            ...
            pass
    vals.append(round(float(p_enter), 4))
    vals = sorted(set(vals))
    return vals


def threshold_key(th: float) -> str:
    return f"prob_ge_{int(round(th * 1000)):03d}bp"


def near_miss_bounds(p_enter: float, width: float) -> tuple[float, float]:
    lower = max(0.0, p_enter - width)
    upper = p_enter
    return lower, upper
...
def apply_symbol_override(cfg: dict, selection_path: Path, symbol_override: str | None) -> dict:
    ...
    sort_cols = [c for c in ["avg_daily_return_on_notional", "median_daily_return_on_notional", "positive_day_ratio", "avg_daily_pnl", "pr_auc", "total_pnl"] if c in rows.columns]
    if sort_cols:
    ...
    cfg2["best_backtest_stats"] = {
        ...
        "avg_daily_pnl": float(row.get("avg_daily_pnl", 0.0)),
        "median_daily_pnl": float(row.get("median_daily_pnl", 0.0)),
        "worst_day_pnl": float(row.get("worst_day_pnl", 0.0)),
        "best_day_pnl": float(row.get("best_day_pnl", 0.0)),
        "positive_day_ratio": float(row.get("positive_day_ratio", 0.0)),
        "avg_daily_return_on_account": float(row.get("avg_daily_return_on_account", row.get("avg_daily_return_pct", 0.0))),
        "avg_daily_return_on_notional": float(row.get("avg_daily_return_on_notional", 0.0)),
        "median_daily_return_on_notional": float(row.get("median_daily_return_on_notional", 0.0)),
        "worst_day_return_on_notional": float(row.get("worst_day_return_on_notional", 0.0)),
        "best_day_return_on_notional": float(row.get("best_day_return_on_notional", 0.0)),
        "total_pnl": float(row.get("total_pnl", 0.0)),
        "total_trades": int(row.get("total_trades", 0)),
        "avg_trades_per_day": float(row.get("avg_trades_per_day", 0.0)),
        "avg_daily_notional_used": float(row.get("avg_daily_notional_used", 0.0)),
        "median_daily_notional_used": float(row.get("median_daily_notional_used", 0.0)),
        "avg_entry_notional": float(row.get("avg_entry_notional", 0.0)),
        "eval_days_used": int(row.get("eval_days_used", 0)),
        "train_days": int(row.get("train_days", 10)),
        "equity_final": float(row.get("equity_final", cfg2.get("initial_cash", 100000))),
    }
...
class SessionMetrics:
    ...
    max_prob_seen: float = 0.0
    min_prob_seen: float = 1.0
    prob_sum: float = 0.0
...
def can_open_new_position(now: datetime, entry_start: dtime, market_close: dtime) -> bool:
    t = now.astimezone(NY).time()
    return entry_start <= t <= market_close


def compute_hold_minutes(entry_time: datetime | None, now: datetime) -> int:
    if entry_time is None:
        return 0
    if entry_time.tzinfo is None:
        entry_time = entry_time.replace(tzinfo=NY)
    return max(0, int((now.astimezone(NY) - entry_time.astimezone(NY)).total_seconds() // 60))
...
def sync_position_state(
    ...
    try:
        ...
        return previous_state if previous_state is not None else PositionState()
    ...
    for pos in positions:
        if getattr(pos, "symbol", None) == symbol:
            ...
            if previous_state is not None and previous_state.in_pos and previous_state.qty == qty:
                entry_time = previous_state.entry_time
                entry_bar_time = previous_state.entry_bar_time
                entry_order_id = previous_state.entry_order_id
                entry_signal_prob = previous_state.entry_signal_prob
            ...
            return PositionState(
                ...
                entry_signal_prob=entry_signal_prob,
            )
    return PositionState()
...
def append_equity_snapshot(
    ...
    if state.in_pos and state.qty > 0:
        px = state.entry_px if state.entry_px is not None else latest_close
        current_notional = abs(float(px) * int(state.qty))
    return_on_notional = 0.0 if current_notional == 0 else total_pnl / current_notional
    new_peak = max(running_peak_equity, equity)
    drawdown = equity - new_peak
    row = {
        "ts": now.isoformat(),
        "symbol": symbol,
...
def format_status_line(
    ...
    metrics: SessionMetrics,
    event_note: str = "",
    allowed_entry: bool | None = None,
) -> str:
    status = "IN_POS" if state.in_pos else "FLAT"
    entry_px = "-" if state.entry_px is None else f"{state.entry_px:.4f}"
    hold_minutes = compute_hold_minutes(state.entry_time, now) if state.in_pos else 0
    parts = [
        f"[{now.strftime('%Y-%m-%d %H:%M:%S')}]",
        symbol,
        f"close={latest_close:.4f}",
        f"prob={prob:.3f}",
        f"p_enter={p_enter:.3f}",
        f"signal={'YES' if raw_signal else 'no'}",
        f"near_miss={'YES' if near_miss else 'no'}",
        f"state={status}",
        f"entry_px={entry_px}",
        f"hold_minutes={hold_minutes}",
        f"raw_signals={metrics.raw_signal_opportunities}",
        f"entries={metrics.actual_entries}",
    ]
    if allowed_entry is not None:
        parts.append(f"allowed_entry={'YES' if allowed_entry else 'no'}")
    if event_note:
        parts.append(f"event={event_note}")
    return " | ".join(parts)
...
def build_daily_review_summary(
    *,
    ...
    eod_exit_count = sum(1 for t in closed if t.exit_reason == "EOD")

    unrealized_pnl = compute_unrealized_pnl(current_state, latest_close)
    net_pnl = realized_pnl + unrealized_pnl
    ending_equity = day_start_equity + net_pnl

    total_trade_notional = sum((t.entry_px or 0.0) * (t.qty or 0) for t in closed)
    avg_trade_return_on_notional = 0.0 if total_trade_notional == 0 else realized_pnl / total_trade_notional

    avg_prob_today = 0.0 if metrics.evaluated_bars == 0 else metrics.prob_sum / metrics.evaluated_bars
    max_prob_today = metrics.max_prob_seen if metrics.evaluated_bars else 0.0
    min_prob_today = metrics.min_prob_seen if metrics.evaluated_bars else 0.0

    if equity_snapshots:
        eq_df = pd.DataFrame(equity_snapshots)
        avg_daily_return_on_notional = safe_float(eq_df["return_on_notional"].dropna().mean(), 0.0) if "return_on_notional" in eq_df.columns else 0.0
        max_intraday_drawdown = safe_float(eq_df["drawdown"].min(), 0.0) if "drawdown" in eq_df.columns else 0.0
    else:
        avg_daily_return_on_notional = 0.0
        max_intraday_drawdown = 0.0

    near_low, near_high = near_miss_bounds(p_enter, near_miss_width)
    if metrics.raw_signal_opportunities == 0 and metrics.actual_entries == 0:
        if max_prob_today < near_low:
            diagnosis = f"No entry signals. Best probability {max_prob_today:.3f} stayed well below p_enter {p_enter:.3f}."
        elif max_prob_today < p_enter:
            diagnosis = f"No entry signals. Best probability {max_prob_today:.3f} came close but did not reach p_enter {p_enter:.3f}."
        else:
            diagnosis = "Signals were seen but no entries completed. Check broker/order flow logs."
    else:
        diagnosis = "Signals or entries occurred during the session."

    review_comment = diagnosis
    if avg_daily_return_on_notional > 0:
        review_comment += " Return on notional finished positive today."
    elif avg_daily_return_on_notional < 0:
        review_comment += " Return on notional finished negative today."
    else:
        review_comment += " Return on notional finished flat today."
    ...
    summary = {
        ...
        "qty": current_state.qty if current_state.in_pos and current_state.qty > 0 else int(cfg.get("qty", 0)),
        ...
        "avg_pnl_per_trade": 0.0 if total_trades == 0 else float(np.mean(pnl_values)),
        "avg_hold_minutes": 0.0 if not hold_values else float(np.mean(hold_values)),
        "best_trade_pnl": 0.0 if not pnl_values else float(np.max(pnl_values)),
        "worst_trade_pnl": 0.0 if not pnl_values else float(np.min(pnl_values)),
        "raw_signals": metrics.raw_signal_opportunities,
        "blocked_signals": metrics.blocked_signals_while_in_position,
        "actual_entries": metrics.actual_entries,
        "completed_exits": metrics.completed_exits,
        "tp_exits": tp_exit_count,
        "sl_exits": sl_exit_count,
        "time_exits": time_exit_count,
        "eod_exits": eod_exit_count,
        ...
        "evaluated_bars": metrics.evaluated_bars,
        "max_prob_today": max_prob_today,
        "min_prob_today": min_prob_today,
        "avg_prob_today": avg_prob_today,
        "near_miss_count": metrics.near_miss_count,
        "near_miss_lower": near_low,
        "near_miss_upper": near_high,
        "current_trade_id": current_trade_id or "",
...
def save_session_summary(
    ...
    row = {
        ...
        "symbol": symbol,
        **asdict(metrics),
        "avg_prob_seen": 0.0 if metrics.evaluated_bars == 0 else metrics.prob_sum / metrics.evaluated_bars,
        "selection_avg_daily_return_on_notional": cfg.get("best_backtest_stats", {}).get("avg_daily_return_on_notional"),
...
def main() -> None:
    ...
    ap.add_argument("--selection_json", default="selector_out_416/best_etf_selection_*.json")
    ...
    ap.add_argument("--model_out", default="final_model_416.joblib")
    ap.add_argument("--logs_dir", default="live_logs_416")
    ...
    ap.add_argument("--log_all_probs", action="store_true", help="Store every minute probability row, not only near misses or signals")
    ap.add_argument("--prob_thresholds", default="0.45,0.50,0.55,0.60,0.65", help="Comma-separated probability thresholds to count")
    ap.add_argument("--near_miss_width", type=float, default=0.05, help="Near-miss band below p_enter, such as 0.05 for [p_enter-0.05, p_enter)")
    ap.add_argument("--p_enter", type=float, default=None, help="Optional live override for entry threshold. Example: 0.60")
    ap.add_argument("--entry_start", default="10:00", help="Earliest NY time allowed for a new BUY, such as 10:00")
    args = ap.parse_args()
    ...
    if args.p_enter is not None:
        p_enter = float(args.p_enter)
    ...
    threshold_values = build_thresholds(args.prob_thresholds, p_enter)
    threshold_counts = {threshold_key(th): 0 for th in threshold_values}

    market_open = dtime(*map(int, cfg["market_hours"]["open"].split(":")))
    market_close = dtime(*map(int, cfg["market_hours"]["close"].split(":")))
    entry_start = parse_hhmm(args.entry_start)
    ...
    equity_log = CsvLogger(
        ...
        [
            ...
            "realized_pnl", "unrealized_pnl", "total_pnl", "equity", "current_notional", "return_on_notional",
            "running_peak_equity", "drawdown", "current_trade_id",
            "evaluated_bars", "raw_signal_count", "blocked_signal_count", "actual_entries", "completed_exits"
        ],
    )
    prob_cols = ["ts", "symbol", "bar_ts", "close_px", "prob", "p_enter", "raw_signal", "near_miss", "in_position", "entry_px", "qty"] + [threshold_key(th) for th in threshold_values]
    prob_log = CsvLogger(logs_dir / f"live_probs_{symbol}_{run_date}.csv", prob_cols)
    session_summary_path = logs_dir / f"live_summary_{symbol}_{run_ts}.csv"
    daily_review_path = logs_dir / "daily_review_summary.csv"

    state = sync_position_state(trading, symbol, previous_state=None)
    metrics = SessionMetrics()
        ...
        while True:
            ...
            if not within_market_hours(now, market_open, market_close):
                review_row = build_daily_review_summary(
                    ...
                    p_enter=p_enter,
                    near_miss_width=args.near_miss_width,
                )
                review_row["paper"] = args.paper
                save_daily_review_summary(daily_review_path, review_row)
                save_session_summary(session_summary_path, session_start, now, symbol, metrics, cfg, threshold_counts, p_enter)
                print(f"[{now.strftime('%Y-%m-%d %H:%M:%S')}] Outside market hours. Waiting for next minute...")
            ...
            if df_raw.empty or len(df_raw) < 200:
                print(f"[{now.strftime('%Y-%m-%d %H:%M:%S')}] Waiting: fetched only {len(df_raw)} rows for {symbol}.")
            ...
            try:
                ...
                print(f"[{now.strftime('%Y-%m-%d %H:%M:%S')}] skipped retrain/infer prep: {e}")
                time.sleep(minute_sleep_seconds(now))
                continue
            ...
            state = sync_position_state(trading, symbol, fallback_px=state.entry_px, previous_state=state)
            ...
            if (model is None) or (last_retrain is None) or ((now - last_retrain).total_seconds() >= args.retrain_minutes * 60):
                ...
                print(f"[{now.strftime('%Y-%m-%d %H:%M:%S')}] RETRAIN | rows={len(X_train)} | saved={args.model_out}")
            ...
            metrics.max_prob_seen = max(metrics.max_prob_seen, prob)
            metrics.min_prob_seen = min(metrics.min_prob_seen, prob)
            metrics.prob_sum += prob

            for th in threshold_values:
                if prob >= th:
                    threshold_counts[threshold_key(th)] += 1

            near_low, near_high = near_miss_bounds(p_enter, args.near_miss_width)
            near_miss = (prob >= near_low) and (prob < near_high)
            if near_miss:
                metrics.near_miss_count += 1

            raw_signal = prob >= p_enter
            entry_allowed = can_open_new_position(now, entry_start, market_close)
            event_note = ""
            if raw_signal:
                metrics.raw_signal_opportunities += 1
                event_note = "RAW_SIGNAL"
            ...
            prob_row = {
                "ts": now.isoformat(),
                "symbol": symbol,
                "bar_ts": latest_bar_ts.isoformat(),
                "close_px": latest_close,
                "prob": prob,
                "p_enter": p_enter,
            ...
            for th in threshold_values:
                prob_row[threshold_key(th)] = prob >= th
            if args.log_all_probs or raw_signal or near_miss:
                prob_log.write(prob_row)

            if raw_signal and (not state.in_pos) and (not entry_allowed):
                event_note = "BLOCKED_TIME_FILTER"
                log_event(event_log, "BLOCKED_TIME_FILTER", now, symbol, {
                    "prob": prob,
                    "close_px": latest_close,
                    "entry_px": "",
                    "exit_px": "",
                    "qty": qty,
                    "reason": f"before_entry_start_{args.entry_start}",
                    "minutes_in": "",
                    "tp_px": "",
                    "sl_px": "",
                    "raw_signal_count": metrics.raw_signal_opportunities,
            ...
            if (not state.in_pos) and raw_signal and entry_allowed:
                ...
                print(f"[{now.strftime('%Y-%m-%d %H:%M:%S')}] BUY | prob={prob:.3f} | entry_px={entry_px:.4f} | qty={qty} | order_id={order_id}")
            ...
            if state.in_pos and state.entry_px is not None:
                ...
                if state.entry_bar_time is None:
                    state.entry_bar_time = latest_bar_ts

                minutes_in = compute_hold_minutes(state.entry_time, now)
                entry_px = state.entry_px
                tp_px = entry_px * (1 + tp)
                sl_px = entry_px * (1 - sl) if sl is not None else None

                infer_after = infer[infer.index >= state.entry_bar_time].copy()
                if len(infer_after) > 0:
                    hi = float(infer_after["High"].max())
                    lo = float(infer_after["Low"].min())

                    exit_reason = None
                    if minutes_in >= horizon:
                        exit_reason = "TIME"
                    elif sl_px is not None and lo <= sl_px:
                        exit_reason = "SL"
                    elif hi >= tp_px:
                        exit_reason = "TP"
                    ...
                    if exit_reason is not None:
                        ...
                        event_note = f"SELL_{exit_reason}"
                        print(f"[{now.strftime('%Y-%m-%d %H:%M:%S')}] SELL | reason={exit_reason} | minutes_in={minutes_in} | exit_px={exit_fill_px:.4f} | pnl=${trade_pnl:.2f}")
                        ...
                        if current_trade is None:
                            current_trade = TradeRecord(
                                ...
                                entry_bar_ts=state.entry_bar_time.isoformat() if state.entry_bar_time else latest_bar_ts.isoformat(),
                                entry_px=entry_px,
            ...
            snap, running_peak_equity = append_equity_snapshot(
            ...
            print(format_status_line(now, symbol, latest_close, prob, p_enter, raw_signal, near_miss, state, metrics, event_note, allowed_entry=entry_allowed))
