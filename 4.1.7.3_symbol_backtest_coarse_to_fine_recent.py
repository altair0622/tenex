    ...
    for idx, (horizon, tp, sl, p_enter) in enumerate(param_grid, start=1):
        ...
        try:
            stats, daily_rows = base.walkforward_evaluate(
                ...
                model_params=MODEL_PARAMS_4173,
                horizon=horizon,
                tp=tp,
                sl=sl,
                p_enter=p_enter,
...
def make_selection_json(
    ...
    return {
        "version": VERSION,
        "selection_mode": "single_symbol_recent_recency_weighted_coarse_to_fine_featureboost_singlethread_rf",
        ...
        "selection_reason": {
            ...
            "summary": (
                ...
                f"4.1.7.3 fixes the sklearn parallel warning by forcing RandomForest training to single-thread mode "
                f"inside this optimizer."
            ),
...
def main() -> None:
    ap = argparse.ArgumentParser(description="4.1.7.3 recent-window coarse-to-fine optimizer for a single symbol")
    ap.add_argument("--symbol", default="TQQQ")
    ap.add_argument("--days", type=int, default=30)
    ap.add_argument("--feed", default="iex")
    ap.add_argument("--out_dir", default="selector_out_4173")
