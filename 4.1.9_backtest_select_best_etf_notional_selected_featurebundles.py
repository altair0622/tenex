#!/usr/bin/env python3
"""
4.1.9_backtest_select_best_etf_notional_selected_featurebundles.py
...
def add_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rolling_low_15 = d["Low"].rolling(window=15, min_periods=15).min()
    rolling_high_15 = d["High"].rolling(window=15, min_periods=15).max()
    rolling_span_15 = (rolling_high_15 - rolling_low_15).replace(0, np.nan)
    d["range_pos_15"] = (d["Close"] - rolling_low_15) / rolling_span_15
...
def main() -> None:
    ...
    ap.add_argument("--out_dir", default="selector_out_419")
    ...
    selection_json = {
        "version": "4.1.9",
        "selection_mode": "single_best_etf_daily_return_on_notional_walkforward_selected_featurebundles",
