...
def add_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    d["oc_body_pct"] = (d["Close"] - d["Open"]) / d["Open"].replace(0, np.nan)

    d["vwap_dist"] = (d["Close"] - d["vwap"]) / d["vwap"].replace(0, np.nan)
    d["ema_dist"] = (d["Close"] - d["EMA"]) / d["EMA"].replace(0, np.nan)
    d["macd_hist"] = d["MACD"] - d["Signal_Line"]

    rolling_high_5 = d["High"].rolling(5).max()
    rolling_low_5 = d["Low"].rolling(5).min()
    rolling_high_15 = d["High"].rolling(15).max()
    rolling_low_15 = d["Low"].rolling(15).min()

    d["breakout_above_5"] = (d["Close"] > rolling_high_5.shift(1)).astype(float)
    d["breakdown_below_5"] = (d["Close"] < rolling_low_5.shift(1)).astype(float)
    d["breakout_above_15"] = (d["Close"] > rolling_high_15.shift(1)).astype(float)
    d["range_pos_15"] = (d["Close"] - rolling_low_15) / (rolling_high_15 - rolling_low_15).replace(0, np.nan)

    d["volume_sma_5"] = d["Volume"].rolling(5).mean()
    d["volume_sma_15"] = d["Volume"].rolling(15).mean()
    d["volume_spike_5"] = d["Volume"] / d["volume_sma_5"].replace(0, np.nan)
    d["volume_spike_15"] = d["Volume"] / d["volume_sma_15"].replace(0, np.nan)

    d["realized_vol_5"] = d["ret_1"].rolling(5).std()
    d["realized_vol_15"] = d["ret_1"].rolling(15).std()

    minutes_from_open = (d.index.hour * 60 + d.index.minute) - (10 * 60)
    d["minutes_from_open"] = minutes_from_open.astype(float)
    d["morning_session"] = (d["minutes_from_open"] <= 90).astype(float)
    d["midday_session"] = ((d["minutes_from_open"] > 90) & (d["minutes_from_open"] <= 210)).astype(float)
    d["late_session"] = (d["minutes_from_open"] > 210).astype(float)
...
FEATURE_COLS = [
    ...
    "vwap_dist", "ema_dist", "macd_hist",
    "breakout_above_5", "breakdown_below_5", "breakout_above_15", "range_pos_15",
    "volume_spike_5", "volume_spike_15",
    "realized_vol_5", "realized_vol_15",
    "minutes_from_open", "morning_session", "midday_session", "late_session"
]
...
def walkforward_evaluate(
    ...
    valid_pr_aucs = [x for x in pr_aucs if pd.notna(x)]
    stats = {
...
def main() -> None:
    ...
    ap.add_argument("--out_dir", default="selector_out_417")
    ...
    selection_json = {
        "version": "4.1.7",
        "selection_mode": "single_best_etf_daily_return_on_notional_walkforward_highconviction_featureboost",
