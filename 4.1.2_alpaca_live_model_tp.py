...
Goal:
- Load best_params JSON produced by the 4.1.2 backtest
- Fetch recent minute bars from Alpaca
- Build the same features
- Retrain a live model on a rolling recent window
- Auto trade one symbol using the best backtest parameters

What is improved vs 4.1.1:
- Uses actual filled average price when available
- Syncs with broker positions on startup and during the loop
- Reads backtest cost information for logging and comparison
- Tracks raw signals, blocked signals, and actual entries in live session metrics
- Writes event logs and session summary CSV files
- Paces the loop to minute boundaries instead of a fixed 5 second sleep
...
def expected_cost_bps_from_cfg(cfg: dict) -> float:
    strat = cfg.get("best_strategy_params", {})
    fee_bps = float(strat.get("fee_bps", 0.0) or 0.0)
...
def save_session_summary(path: Path, session_start: datetime, now: datetime, symbol: str, metrics: SessionMetrics, cfg: dict) -> None:
    row = {
        ...
        "symbol": symbol,
        **asdict(metrics),
        "backtest_expected_cost_bps": expected_cost_bps_from_cfg(cfg),
...
def main() -> None:
    ...
    ap.add_argument("--retrain_minutes", type=int, default=60, help="How often to retrain the model")
    ap.add_argument("--model_out", default="final_model.joblib")
    ap.add_argument("--logs_dir", default="live_logs")
    ...
    expected_cost_bps = expected_cost_bps_from_cfg(cfg)
    ...
    print({
        ...
        "qty": args.qty,
        "paper": args.paper,
        "expected_cost_bps_from_backtest": expected_cost_bps,
    ...
    try:
        while True:
            ...
            if (not state.in_pos) and raw_signal:
                buy_order = place_market_buy(trading, symbol, args.qty)
            ...
            if state.in_pos and state.entry_px is not None:
                ...
                if len(df_after) > 0:
                    ...
                    if exit_reason is not None:
                        sell_order = place_market_sell(trading, symbol, state.qty if state.qty > 0 else args.qty)
                        ...
                        log_event(event_log, "SELL", now, symbol, {
                            ...
                            "qty": state.qty if state.qty > 0 else args.qty,
                            "reason": exit_reason,
